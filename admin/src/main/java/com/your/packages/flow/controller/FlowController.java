...
public class FlowController {
    ...
    public R<PageResult<FlowPageVO>> getFlowPage(
            PageParam pageParam, FlowQO flowQO) {
    ...
    public R save(@RequestBody Flow flow) {
        return flowService.save(flow) ?
                R.ok() : R.failed(BaseResultCode.UPDATE_DATABASE_ERROR, "新增夫路失败");
    }
    ...
    public R updateById(@RequestBody Flow flow) {
        return flowService.updateById(flow) ?
                R.ok() : R.failed(BaseResultCode.UPDATE_DATABASE_ERROR, "修改夫路失败");
    }
    ...
    public R removeById(@PathVariable("id") Long id) {
        return flowService.removeById(id) ?
                R.ok() : R.failed(BaseResultCode.UPDATE_DATABASE_ERROR, "通过id删除夫路失败");
    }
